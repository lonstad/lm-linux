			...
			int io)
{
	...
	if (addr > DS2781_DATA_SIZE || addr < 0) {
		count = 0;
		dev_err(dev, "%s: addr %d /count %d out of range\n", __func__, addr, count);
		goto out;
	}
	if (addr + count > DS2781_DATA_SIZE)
		count = DS2781_DATA_SIZE - addr;

	//dev_info(dev, "%s: %s a=%d, count=%d", __func__, io ? "Wr" : "Rd", addr, count);
	w1_next_pullup(sl->master, 10);
...
static int w1_ds2781_eeprom_cmd(struct device *dev, int addr, int cmd)
{
	struct w1_slave *sl = container_of(dev, struct w1_slave, dev);
...
int w1_ds2781_store_eeprom(struct device *dev, int addr)
{
	return w1_ds2781_eeprom_cmd(dev, addr, W1_DS2781_COPY_DATA);
}

int w1_ds2781_recall_eeprom(struct device *dev, int addr)
{
	return w1_ds2781_eeprom_cmd(dev, addr, W1_DS2781_RECALL_DATA);
}

static ssize_t w1_ds2781_read_bin(struct file *filp, struct kobject *kobj,
...
EXPORT_SYMBOL(w1_ds2781_store_eeprom);
EXPORT_SYMBOL(w1_ds2781_recall_eeprom);
