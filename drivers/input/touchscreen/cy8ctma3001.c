...
static int __devinit cy8_probe(struct i2c_client *client,
 ...
 * ************************************************************************* */
static const struct i2c_device_id cy8_id[] = { { "cy8ctma300", 0 }, { } };

MODULE_DEVICE_TABLE(i2c, cy8_id);

static struct i2c_driver cy8_driver = { .driver = { .name = "cy8ctma300",
		.owner = THIS_MODULE, }, .probe = cy8_probe, .remove =
		__devexit_p(cy8_remove), .suspend = cy8_suspend, .resume = cy8_resume,
		.id_table = cy8_id, };
...
void cy8_xy_worker(struct work_struct *work) {
	...
	retval = i2c_smbus_read_i2c_block_data(ts->client, 0x02,
			sizeof(struct xydata_t), (u8 *) &xy_data);
	if (retval < 0) {
		goto exit_xy_worker;
	}

	if (ts->platform_data->flags & CY8F_XY_AXIS_FLIPPED) { /* We need to swap X & Y coordinates */
		tmp = xy_data.x;
		xy_data.x = xy_data.y;
		xy_data.y = tmp;
	}
	...
	if (ts->platform_data->flags & CY8F_REVERSE_X)
		xy_data.x = ts->platform_data->maxx - xy_data.x;

	if (ts->platform_data->flags & CY8F_REVERSE_Y)
...
#undef DEBUG
	...
	exit_xy_worker: if (ts->client->irq == 0) {
...
static int cy8_initialize(struct i2c_client *client, struct cy8 *ts) {
	...
	error_free_device: input_free_device(input_device);
